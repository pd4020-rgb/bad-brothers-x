...
| **모바일 세로 소형** | `max-width: 768px` | 햄버거 메뉴 활성화, 패딩 축소 |
| **모바일 가로** | `max-height: 500px + landscape` | 2열 그리드 복원, 인라인 네비 |
...
| 섹션 | 확인 항목 | 규격 |
|------|-----------|------|
| **헤더** | 로고 + 햄버거 버튼 (≤768px) | padding: 16px |
| **메뉴** | 오른쪽 드로어 슬라이드 | width: 70%, max-width: 280px, 뒤 반투명 |
| **히어로** | 영상 먼저(order:1) → 텍스트+버튼(order:2) | 1열, 중앙 정렬 |
...
| **글씨** | 타이틀 3rem, 본문 0.95rem | 들쭉날쭉 금지 |
| **버튼** | 소형화 (10px 20px, 0.85rem) | 중앙 정렬 |
...
| 섹션 | 확인 항목 | 규격 |
|------|-----------|------|
| **헤더** | 인라인 네비 (햄버거 숨김) | font: 0.65rem, gap: 6px, 로고 1.1rem |
