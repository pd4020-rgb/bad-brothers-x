...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas) {
        ...
        function getThemeColor() {
            ...
            return parseColor(textWhite) || { r: 28, g: 28, b: 28 };
        }
        ...
        // Use local path to avoid Safari file:/// CORS parent directory restrictions
        ...
        function buildMask() {
            ...
            const cropH = imgH * 0.80;
            const cropW = imgW;
            const cropAspect = cropW / cropH;

            // Center the face on all screens
            const targetW = Math.min(width * 1.5, 1400);
        ...
        function sampleTargets() {
            ...
            const cropH = imgH * 0.80, cropW = imgW;
            const cropAspect = cropW / cropH;
            
            const targetW = Math.min(width * 1.5, 1400);
            const targetH = height;
            let dw, dh, dx, dy;
            if (cropAspect > targetW / targetH) { dh = targetH; dw = dh * cropAspect; }
            else { dw = targetW; dh = dw / cropAspect; }
            dx = (width - dw) / 2;
            dy = (height - dh) / 2 - height * 0.05;
